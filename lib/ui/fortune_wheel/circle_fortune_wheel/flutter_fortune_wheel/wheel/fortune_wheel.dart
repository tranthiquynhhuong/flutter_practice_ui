...
class FortuneWheel extends HookWidget implements FortuneWidget {
  ...
  /// {@macro flutter_fortune_wselectedheel.FortuneWidget.indicators}
  ...
  BehaviorSubject<bool> bsInitDone = BehaviorSubject.seeded(false);
  ...
  FortuneWheel({
    ...
    this.onFling,
        ...
        assert(selected.value >= 0 && selected.value < items.length),
        assert(curve != null),
        super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    selected.listen((value) async {
      if (value == 0 && bsInitDone.value == false) {
        bsInitDone.add(true);
        return;
      }
            ...
            AnimatedBuilder(
              ...
              builder: (context, _) {
                ...
                return Transform.rotate(
                  angle: -2 * Math.pi * (selected.value / items.length) +
